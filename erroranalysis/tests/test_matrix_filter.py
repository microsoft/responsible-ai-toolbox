...
class TestMatrixFilter(object):
    ...
    def test_matrix_filter_iris_filters(self):
        ...
        model_task=ModelTask.CLASSIFICATION
        run_error_analyzer_on_models(X_train, y_train, X_test,
            y_test, feature_names, model_task, filters=filters)
...
def run_error_analyzer_on_models(X_train,
    ...
    for model in models:
        ...
        run_error_analyzer(model, X_test, y_test, feature_names,
            ...
            filters=filters, composite_filters=composite_filters)
...
def run_error_analyzer(model,
    ...
    if filters is not None or composite_filters is not None:
        validation_data = filter_from_cohort(X_test, filters,
            composite_filters, error_analyzer.feature_names, y_test,
            error_analyzer.categorical_features, error_analyzer.categories)
    ...
    if metric == Metrics.ERROR_RATE:
        ...
        raise NotImplementedError("Metric {} validation not supported yet".format(metric))
    validate_matrix(json_matrix, expected_count, expected_error, metric=metric)
...
def validate_matrix(json_matrix, exp_total_count,
    ...
    validate_matrix_metric(json_matrix, exp_total_count,
        exp_total_error, metric, num_cat1, num_cat2)
    ...
    if metric == Metrics.ERROR_RATE:
        ...
        for i in range(num_cat1):
            for j in range(num_cat2):
                ...
                total_count += count
                total_metric_value += json_matrix[MATRIX][i][j][METRIC_VALUE] * count
