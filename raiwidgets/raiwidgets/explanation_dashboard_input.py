from .explanation_constants import ExplanationDashboardInterface, WidgetRequestResponseConstants
    ...
    def __init__(
        ...
        if dataset is not None and model is not None:
            try:
                ...
                ex_str = _format_exception(ex)
                msg = "Model does not support predict method for given dataset type, inner error: {}".format(
                    ex_str)
            ...
            try:
                ...
                ex_str = _format_exception(ex)
                raise ValueError(
                    "Model prediction output of unsupported type, inner error: {}".format(ex_str))
        ...
        if true_y is not None and len(true_y) == row_length:
            self.dashboard_input[ExplanationDashboardInterface.TRUE_Y] = self._convert_to_list(
                true_y)
        ...
        if local_explanation is not None:
            try:
                ...
                ex_str = _format_exception(ex)
                raise ValueError(
                    "Unsupported local explanation type, inner error: {}".format(ex_str))
            if list_dataset is not None:
                ...
                if len(local_dim) == 2 and (local_dim[1] != feature_length or local_dim[0] != row_length):
                    raise ValueError(
                        "Shape mismatch: local explanation length differs from dataset")
        ...
        if local_explanation is None and global_explanation is not None:
            try:
                ...
                ex_str = _format_exception(ex)
                raise ValueError(
                    "Unsupported global explanation type, inner error: {}".format(ex_str))
        ...
        if model is not None and hasattr(model, SKLearn.PREDICT_PROBA) \
            ...
            try:
                ...
                ex_str = _format_exception(ex)
                raise ValueError(
                    "Model predict_proba output of unsupported type, inner error: {}".format(ex_str))
