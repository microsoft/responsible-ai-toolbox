...
try
{
    ...
    $codepath = [System.IO.Path]::Combine($rootpath, "error_analysis")
    ...
    New-Item "docbuild" -ItemType Directory -Force
    $confpath = [System.IO.Path]::Combine($docconfigpath, "conf.py")
    $indexpath = [System.IO.Path]::Combine($docconfigpath, "index.rst")
    Copy-Item $confpath -Destination "$docbuildpath" -Recurse -Force
    Copy-Item $indexpath -Destination "$docbuildpath" -Recurse -Force
