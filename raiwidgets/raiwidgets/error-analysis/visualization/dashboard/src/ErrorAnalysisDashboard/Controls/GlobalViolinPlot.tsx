...
export class GlobalViolinPlot extends React.PureComponent<IGlobalViolinPlotProps, IGlobalViolinPlotState> {
    ...
    (jointDataset: JointDataset, metadata: IExplanationModelMetadata, cohort: Cohort, sortVector: number[], selectedOption: IComboBoxOption): IPlotlyProperty => {
        ...
        sortVector.forEach((featureIndex, xIndex) => {
            const featureImportance = cohort.unwrap(JointDataset.ReducedLocalImportanceRoot + featureIndex.toString());
            const data = {...baseData};
    ...
    public render(): React.ReactNode {
        if (this.state.plotlyProps === undefined) {
            const plotlyProps = GlobalViolinPlot.buildPlotlyProps(this.props.jointDataset, this.props.metadata, this.props.cohort, this.props.sortVector, undefined);
