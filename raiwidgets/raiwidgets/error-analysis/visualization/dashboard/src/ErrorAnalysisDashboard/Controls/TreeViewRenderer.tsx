...
export class TreeViewRenderer extends React.PureComponent<ITreeViewRendererProps, ITreeViewRendererState> {
    ...
    private fetchTreeNodes() {
        ...
        this.props.getTreeNodes(this.props.selectedFeatures, new AbortController().signal)
            .then((result) => {
                this.onResize()
                this.clearSelection()
                this.forceUpdate()
    ...
    public render(): React.ReactNode {
        ...
        const max = d3max(this.state.root.descendants(), d => d.data.error / d.data.size)
        ...
        var nodeData = this.state.root.descendants().map(d => {
          ...
          const calcHoverText = `Error Rate: ${((d.data.error / d.data.size) * 100).toFixed(
            2
          )}%\r\nError Coverage: ${((d.data.error / this.state.rootErrorSize) * 100).toFixed(
            2
          )}%\r\nInstances: ${d.data.size}\r\n${d.data.pathFromRoot}${this.skippedInstances(d)}`
  
          let heatmapStyle: { fill: string } = {"fill": ErrorAvgColor}
  
          if (d.data.error / d.data.size > this.state.rootLocalError * ErrorRatioThreshold) {
        ...
        return (
          <div className="mainFrame" id="mainFrame">
            <div className="innerFrame">
                <svg ref={SvgOuterFrame} className="SvgOuterFrame" id="SvgOuterFrame" viewBox="0 0 1000 500" onClick={this.bkgClick.bind(this)}>
                <mask id="Mask">
                    <rect className="nodeMask" x="-26" y="-26" width="52" height="52" fill="white" />
                </mask>
                ...
                <g className="details" onClick={e => e.stopPropagation()}>
                    ...
                    <g className="innerOpacityToggle">
                        <rect className="opacityToggleRect" width="280" height="140" fill="transparent"/>
                        <text className="detailLines">{nodeDetail.instanceInfo} [ {nodeDetail.errorInfo} | {nodeDetail.successInfo} ]</text>
                        <g className="opacityToggleCircle">
                        <circle r="26" className="node" style={nodeDetail.errorColor} />
                        <g style={nodeDetail.mask_down} mask="url(#detailMask)" className="nopointer">
                            <circle r="26" className="node" fill="#d2d2d2" style={nodeDetail.mask_up} />
                        </g>
                        <text className="detailPerc">{nodeDetail.globalError}%</text>
                    ...
                    {nodeData.map((node, index) => (
                        <CSSTransition key={node.id} in={true} timeout={200} className="nodes">
                            <g key={node.id} style={node.style} onClick={(e => this.select(index, node, e)).bind(this)}>
                                ...
                                <text textAnchor="middle" className="node_text">{node.data.error} / {node.data.size}</text>
                                <title>{node.hoverText}</title>
                            </g>
                        </CSSTransition>
                    ))}
                </TransitionGroup>
                <TransitionGroup component="g" className="linkLabelsTransitionGroup">
    ...
    public reloadData(treeNodes): void {
        const reloadDataFunc = (state, props) => {
            ...
            return {treeNodes: treeNodes, root: root, rootSize: rootSize, rootErrorSize: rootErrorSize, rootLocalError: rootLocalError}
        };
    ...
    public select(index, node, event): void {
        ...
        const updateSelectedFunc = (state, props) => {
            ...
            return {selected_node: node, nodeDetail: nodeDetail,
                treeNodes: state.treeNodes, root: state.root,
                rootSize: state.rootSize, rootErrorSize: state.rootErrorSize,
