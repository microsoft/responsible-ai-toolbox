                        ...
                        <div className={classNames.statsBox}>
                          <Text block>
                            {localization.formatString(
                              localization.ModelPerformance.accuracy,
                              stats.accuracy.toPrecision(3)
                            )}
                          </Text>
                          <Text block>
                            {localization.formatString(
                              localization.ModelPerformance.precision,
                              stats.precision.toPrecision(3)
                            )}
                          </Text>
                          <Text block>
                            {localization.formatString(
                              localization.ModelPerformance.recall,
                              stats.recall.toPrecision(3)
                            )}
                          </Text>
                          <Text block>
                            {localization.formatString(
                              localization.ModelPerformance.fpr,
                              stats.falsePositiveRate.toPrecision(3)
                            )}
                          </Text>
                          <Text block>
                            {localization.formatString(
                              localization.ModelPerformance.fnr,
                              stats.falseNegativeRate.toPrecision(3)
                            )}
                          </Text>
                        </div>
                      );
    ...
    if (chartProps.yAxis.property === Cohort.CohortKey) {
      ...
      cohorts.forEach((cohort, cohortIndex) => {
        const cohortXs = cohort.unwrap(
          ...
          chartProps.chartType === ChartTypes.Bar
        );
  ...
  private generateMetrics(): any[] {
    ...
    if (this.props.metadata.modelType === ModelTypes.binary) {
      ...
      return sortedCategoricalValues.map((label, labelIndex) => {
        const matchingIndex = (treatAsCategorical
          ? label
          : labelIndex) as string;
