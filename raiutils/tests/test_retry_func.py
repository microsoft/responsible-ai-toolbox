...
class TestRetryFunction:
    ...
    def test_error_with_int_delay(self):
        ...
        with pytest.raises(RuntimeError, match='test failed'):
            retry_function(func, 'test', 'test failed', max_retries=4, retry_delay=1)
    ...
    def test_error_with_zero_delay(self):
        ...
        t_start = time.time()
        with pytest.raises(RuntimeError, match='test failed'):
            retry_function(func, 'test', 'test failed', max_retries=4, retry_delay=0)
        time_taken = time.time() - t_start
    ...
    def test_error_with_float_delay(self):
        ...
        t_start = time.time()
        with pytest.raises(RuntimeError, match='test failed'):
            retry_function(func, 'test', 'test failed', max_retries=4, retry_delay=0.5)
        time_taken = time.time() - t_start
