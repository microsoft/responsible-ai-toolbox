...
def _get_data_balance_measures(dba: DataBalanceAnalysis) -> Optional[Any]:
    if dba.measures:
        # Assume that they have been pre-computed and are valid, so return them
        return dba.measures

    if not dba or dba.df is None or dba.df.empty or not all([dba.cols_of_interest, dba.label_col]):
        return
    ...
    try:
        ...
        feat_measures_df, dist_measures_df, agg_measures_df = _compute_measures(dba=dba)

        unique_values, feature_measures = _transform_feature_measures(df=feat_measures_df)
        distribution_measures = _transform_distribution_measures(df=dist_measures_df)
        ...
        warnings.warn(f"Failed to compute data balance measures for dataset due to {e!r}.")
...
def _compute_measures(
    ...
    feature_measures: pd.DataFrame = FeatureBalanceMeasure(dba.cols_of_interest, dba.label_col).measures(dba.df)
    distribution_measures: pd.DataFrame = DistributionBalanceMeasure(dba.cols_of_interest).measures(dba.df)
    aggregate_measures: pd.DataFrame = AggregateBalanceMeasure(dba.cols_of_interest).measures(dba.df)
...
def _transform_feature_measures(
    ...
    unique_values_set: Dict[str, Set[str]] = {r["feature_name"]: set() for r in rows}
    feature_measures: Dict[str, Dict[str, Dict[str, float]]] = {r["feature_name"]: {} for r in rows}
    ...
    for row in rows:
        ...
        unique_values_set[feature_name].add(class_a)
        unique_values_set[feature_name].add(class_b)
    ...
    unique_values: Dict[str, List[str]] = {k: list(v) for k, v in unique_values_set.items()}

    return unique_values, feature_measures


def _transform_distribution_measures(df: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    """
    Transform the distribution balance measures df into a dictionary
    acceptable by the RAI dashboard
    """
    rows: List[Any] = df.reset_index(drop=True).to_dict(orient="records")
    distribution_measures: Dict[str, Dict[str, float]] = {r["feature_name"]: r for r in rows}
...
def _transform_aggregate_measures(df: pd.DataFrame) -> Dict[str, float]:
    ...
    acceptable by the RAI dashboard
    """
    aggregate_measures: Dict[str, float] = df.reset_index(drop=True).to_dict(orient="records")[0]

    # TODO: This won't be necessary once cols are converted to strings
    aggregate_measures = {k.value: v for k, v in aggregate_measures.items()}
